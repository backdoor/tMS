...
// Тело бота - Блог разработчиков tMS
/*
  Получает JSON
  Выполняет действие
  Отправляет JSON
...
if (!empty($dataJSON)) {
    $dataArray = array();
    addLogMsg("Message","Запущен бот публикации БЛОГА разработчиков");
    ...
    if ($dataArray['events']['type'] == "STREAMLET_PARTICIPANT_ADD") {
        ...
        while ($row = mysql_fetch_assoc($result)) {
            addLogMsg("Message","Бот-найдено поток=".$dataArray['streamlet']['streamID']."(id=".$row['id'].")");
