...
// Создается окно-вкладка "Список потоков за которыми СЛЕЖУ"
function listfollowing(uid)
{
    $.cookie("navigMenuAct", "following");
    ...
    textDataWave+='<table width="100%"><tr><td align="left">'+_lang_nm_Following+'</td><td align="right">';
...
// Обновление списка потоков за которыми слежу
function updateListFollowing() {
    ...
    $.ajax({
        ...
        success: function(obj){
            ...
            //FIXME: Не свой поток можно помечать как СПАМ?
            //allDataListWaves+='<a id="addStream2Spam" class="buttonX box_action" style="color:#47708F;">Спам!</a>';
            allDataListWaves+='<a id="delStream2Following" class="buttonX box_action" style="color:#47708F;">'+_lang_Unfollowing+'</a>';
            ...
            try
            {
                ...
                if(parseInt(dataext.amountWaves)>0) {
                    ...
                    if($.cookie("profileUserActive") == $.cookie("profileUserMe")) {
                        allDataListWaves=allDataListWaves+"<center><p style='font-size:16px; color:#CCC;'>"+_lang_infNoStreamFllwng+"&nbsp;<font size='2px'><a href=\"javascript:goLinkStream('#stream=T1Rr:blip=TXpVeg')\">[?]</a></font>"+"</p></center>";
                    }
                    else {
