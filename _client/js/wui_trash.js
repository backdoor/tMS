...
// Создается окно-вкладка "Список потоков в КОРЗИНЕ"
function listtrash(uid)
{
    $.cookie("navigMenuAct", "trash");
    ...
    textDataWave+='<table width="100%"><tr><td align="left">'+_lang_nm_Trash+'</td><td align="right">';
...
// Обновление ДАННЫХ окна КОРЗИНЫ (uid - номер пользователя)
function updateListTrash() {
    ufid=$.cookie("profileUserActive");
    $.ajax({
        type: "POST",
        url: "serverstream/updateWave.php",
        cache: false,
        data: "ufid="+ufid+"&type=4",
        ...
        success: function(obj){
            ...
            try
            {
                ...
                if(parseInt(dataext.amountWaves)>0) {
                    ...
                    allDataListWaves=allDataListWaves+"<center><p style='font-size:16px; color:#CCC;'>"+_lang_infNoStreamTrash+"&nbsp;<font size='2px'><a href=\"javascript:goLinkStream('#stream=T1Rr:blip=TXpVeg')\">[?]</a></font>"+"</p></center>";
                    }
                    else {
