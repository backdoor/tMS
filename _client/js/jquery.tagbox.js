	...
	$.fn.extend({
		tagbox: function(settings) {
			...
			this.each(function init_box() {
				...
				settings.tag = $('<span class="'+settings.className+'"><label><span></span><input type="text" name="'+settings.name+'" value=" " /><small class="closetag" title="close">x</small></label></span>').get(0);
				setup_tag(settings);
