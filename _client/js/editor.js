    ...
    if (editorVisible) {
        ...
        if ((mylink != null) && (mylink != "")) {
            if (isIE) { //IE
                ...
                if (range.text == '') {
                    range.pasteHTML("<a href='" + mylink + "' class='oembed'>" + mylink + "</a>");
                }
                else
                ...
                if(userSelection.toString().length==0)
                    myeditor.execCommand('inserthtml', false, "<a href='" + mylink + "' class='oembed'>" + mylink + "</a>");
                else
                    myeditor.execCommand("CreateLink", false, mylink);
            }
            else
...
function doLinkWidget() {
    ...
    if (editorVisible) {
        ifm.contentWindow.focus();
        var mylink = prompt("Enter a URL widget:", "http://");
        if ((mylink != null) && (mylink != "")) {
            if (isIE) { //IE
                var range = ifm.contentWindow.document.selection.createRange();
                if (range.text == '') {
                    //range.pasteHTML("<a href='" + mylink + "'>" + mylink + "</a>");
                    range.pasteHTML("<br /><iframe src=\""+$_SYS_SITEPROJECT+"store/widget.php?u=" + mylink + "\" id=\"wg_"+randomValue+"\" onload=\"activateDataWidget('"+randomValue+"')\"></iframe>");
                }
                else
                    myeditor.execCommand("CreateLink", false, mylink);
            }
                ...
                if(userSelection.toString().length==0) {
                    ...
                    myeditor.execCommand('inserthtml', false, "<br /><iframe src=\""+$_SYS_SITEPROJECT+"store/widget.php?u=" + mylink + "\" id=\"wg_"+randomValue+"\" onload=\"activateDataWidget('"+randomValue+"')\"></iframe>");
                    }
                else {
                    myeditor.execCommand("CreateLink", false, mylink); }
            }
...
function doWidget(mylinkwg) {
    var randomValue=Math.random();
    randomValue=randomValue.toString();
    randomValue=randomValue.replace(".", "_");
    myeditor.execCommand('inserthtml', false, "<br /><iframe src=\""+$_SYS_SITEPROJECT+"store/widget.php?u=" + mylinkwg + "\" id=\"wg_"+randomValue+"\" onload=\"activateDataWidget('"+randomValue+"')\"></iframe>");
}
...
function setCCbldSty2(objID, prop, val) {
    switch (prop) {
        case "bc":if (objID != 'none') {document.getElementById(objID).style.backgroundColor = val;};reak;
